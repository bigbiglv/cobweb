...
fn has_apple_music_track(track: Option<&AppleMusicTrackSnapshot>) -> bool {
    track
        .map(|track| {
            track.title.is_some()
...
fn normalize_apple_music_playback_state(
    state: media::AppleMusicPlaybackState,
    track: Option<&AppleMusicTrackSnapshot>,
) -> media::AppleMusicPlaybackState {
    // Note 7: Apple Music 空播放器有时会残留 Playing 状态，但没有任何曲目信息。
    // 这里把“无曲目但 playing”的情况降级成 stopped，避免前端显示暂停按钮。
    if state == media::AppleMusicPlaybackState::Playing && !has_apple_music_track(track) {
        media::AppleMusicPlaybackState::Stopped
    } else {
        state
    }
}

fn play_pause_label(state: media::AppleMusicPlaybackState) -> &'static str {
    match state {
        media::AppleMusicPlaybackState::Playing => "暂停",
        media::AppleMusicPlaybackState::Paused
        | media::AppleMusicPlaybackState::Stopped
        | media::AppleMusicPlaybackState::Unavailable => "播放",
    }
}

pub fn get_feature_groups() -> Vec<FeatureGroup> {
    // Note 8: 这里会实时检查 Apple Music 是否已运行。
    // 如果已运行，前端显示上一曲/播放暂停/下一曲；如果没运行，只显示“打开”按钮。
    let apple_music_running = media::is_apple_music_running();
    let play_pause_label = if apple_music_running {
        let apple_music_track =
            media::get_apple_music_track_info().map(AppleMusicTrackSnapshot::from);
        play_pause_label(normalize_apple_music_playback_state(
            media::get_apple_music_playback_state(),
            apple_music_track.as_ref(),
        ))
    } else {
        "播放"
    };
    let apple_music_features = if apple_music_running {
        vec![FeatureDefinition {
            ...
            description: "".into(),
            mobile_ready: true,
            // Note 9: MediaPlayer 是专门给媒体控制设计的控件。
    ...
    vec![
        FeatureGroup {
            ...
            description: "".into(),
            features: vec![
                // Note 10: 关机和重启都设置 confirm_required: true。
        ...
        FeatureGroup {
            ...
            features: vec![FeatureDefinition {
                ...
                description: "".into(),
                mobile_ready: true,
                // Note 13: Range 表示滑块类控件。min/max/step 限制前端输入范围，
...
pub fn get_feature_snapshot() -> Result<FeatureSnapshot, FeatureServiceError> {
    // Note 14: ? 是 Rust 的错误传播语法。
    // 如果 get_system_volume 失败，函数会立刻返回 Err；成功时才继续组装 FeatureSnapshot。
    let apple_music_running = media::is_apple_music_running();
    let apple_music_track = if apple_music_running {
        media::get_apple_music_track_info().map(AppleMusicTrackSnapshot::from)
    } else {
        None
    };
    let apple_music_playback_state = if apple_music_running {
        normalize_apple_music_playback_state(
            media::get_apple_music_playback_state(),
            apple_music_track.as_ref(),
        )
    } else {
        media::AppleMusicPlaybackState::Unavailable
    };

    Ok(FeatureSnapshot {
        volume_level: system::get_system_volume()?,
        apple_music_running,
        apple_music_playback_state: apple_music_playback_state.as_str().into(),
        apple_music_track,
    })
}
...
pub fn execute_feature_command(
    ...
    match command {
        ...
        FeatureCommand::AppleMusicOpen => {
            // Note 21: 打开 Apple Music 通过 media::open_apple_music 实现。
        ...
        FeatureCommand::AppleMusicPrevious => {
            // Note 22: 上一曲、播放暂停、下一曲都走 Windows 的全局媒体会话 API。
        ...
        FeatureCommand::AppleMusicPlayPause => {
            // Note 23: 播放/暂停是一个 toggle 操作，后端不提前判断目标状态。
        ...
        FeatureCommand::AppleMusicNext => {
            // Note 24: 下一曲和上一曲一样，只是传给媒体模块的枚举值不同。
...
fn apple_music_result(
    ...
    // Note 26: Apple Music 功能执行后会重新读取运行状态、播放状态和歌曲信息。
    ...
    FeatureExecutionResult {
        ...
        message: message.into(),
        volume_level: None,
        apple_music_running: Some(apple_music_running),
        apple_music_playback_state: Some(apple_music_playback_state.as_str().into()),
        apple_music_track,
    }
}
